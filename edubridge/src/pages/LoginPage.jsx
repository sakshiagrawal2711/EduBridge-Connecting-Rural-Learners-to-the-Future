...
export default function LoginPage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // Expect token + user; adjust as per your backend response
      if (res?.token) {
    ...
    <>
      ...
      <main className="pt-24 pb-24 px-4">
        <div className="mx-auto max-w-md bg-white shadow-sm border border-gray-200 rounded-xl p-6">
          ...
          <p className="text-sm text-gray-600 mb-6">Welcome back. Enter your credentials to continue.</p>
          {error && (
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e)=>setPassword(e.target.value)} className="w-full h-11 rounded-md border border-gray-300 px-3 focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="••••••••" />
            </div>
          ...
          <p className="mt-4 text-sm text-gray-600">New here? <a className="text-brand-700 font-medium hover:underline" href="/register">Create an account</a></p>
        </div>
      </main>
    </>
  );
